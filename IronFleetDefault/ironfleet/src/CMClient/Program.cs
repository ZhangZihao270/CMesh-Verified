        ...
        static void Main(string[] args)
{
    ...
    Console.WriteLine("Client process starting " + num_threads + " running for " + experiment_duration + "s ...");
    ...
    var threads = ClientBase.StartThreads<CMClient>(
    num_threads, client_port, initial_seq_no,
    num_req_cnt_a, latency_cnt_ms_a, instant_req_cnt_a).ToArray();
    ...
    else
    {
        ...
        for (int i = 0; i < num_threads; ++i)
        {
            ...
            Console.WriteLine("Client {0} throughput {1} | avg latency ms {2}", i, num_req_cnt_a[i], latency_cnt_ms_a[i] / num_req_cnt_a[i]);
        }
...
//     static void Main(string[] args)
//     {
//         Console.WriteLine("=== C# 消息序列化测试 ===\n");
        
//         // 测试1: ReadMessage (与之前相同)
//         TestReadMessage();
...
//     static void TestReadMessage()
//     {
//         Console.WriteLine("=== 测试1: ReadMessage ===");
        
//         var deps = new Dependency();
//         deps.dep[10] = new VectorClock(new ulong[] { 1, 2, 3 });
//         deps.dep[20] = new VectorClock(new ulong[] { 4, 5, 6 });
        
//         var readMessage = new ReadMessage(100, 200, deps);
//         byte[] readBytes = readMessage.ToBigEndianByteArray();
        
//         Console.WriteLine($"ReadMessage 序列化结果:");
//         Console.WriteLine($"  总长度: {readBytes.Length} 字节");
//         Console.WriteLine($"  十六进制: {BitConverter.ToString(readBytes)}");
//         Console.WriteLine();
//     }
    
//     static void TestWriteMessage()
//     {
//         Console.WriteLine("=== 测试2: WriteMessage ===");
...
//         // 创建Meta (与Dafny端相同: CMeta(100, vc1, deps))
//         var vc1 = new VectorClock(new ulong[] { 1, 2, 3 });
//         var meta = new Meta(100, vc1, deps);
        
//         // 创建Local (与Dafny端相同: map[100 := meta])
//         var local = new Local();
//         local.Data[100] = meta;
        
//         // 创建WriteMessage (与Dafny端相同: CMessage_Write(100, 200, deps, local))
//         var writeMessage = new WriteMessage(100, 200, deps, local);
//         byte[] writeBytes = writeMessage.ToBigEndianByteArray();
        
//         Console.WriteLine($"WriteMessage 序列化结果:");
//         Console.WriteLine($"  总长度: {writeBytes.Length} 字节");
//         Console.WriteLine($"  十六进制: {BitConverter.ToString(writeBytes)}");
//         Console.WriteLine();
//     }
// }
}
