...
module CausalMesh_Message_i {
    ...
    predicate MessageValid(m:Message)
    {
        match m 
            ...
            case Message_Write(_,key_write, deps_write, local) => key_write in Keys_domain && DependencyValid(deps_write) && (forall k :: k in local ==> MetaValid(local[k]) && local[k].key == k)
            case Message_Write_Reply(_,key_wreply, vc_wreply) => key_wreply in Keys_domain && VectorClockValid(vc_wreply) 
